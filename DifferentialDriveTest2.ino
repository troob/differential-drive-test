...
//  volatile int xLoc,
//    yLoc,
//    hLoc; // heading at loc, not heading to loc
//};
//
//LOCATION l1, l2, l3, l4, l5, l6, l7, l8, l9;
//
//const int tourLength = 9;
//
//LOCATION *tour[tourLength] = { &l1, &l2, &l3, &l4, &l5, &l6, &l7, &l8, &l9 };
...
volatile int relativePositionVector,
  angPos,
  angDisp,
  //measRelHeading, // [deg]
  //measAbsHeading, // [deg]
...
void displayCommands()
{
  ...
  Serial.println("wpx: set target position to waypoint x");
  Serial.println();
}
...
void setParams()
{
  setPIDGains(1.125, 0.1, 0.5); // kd may be good at 0.5; init vals tuned roughly
...
//    }
    ...
    {
      setAbsHeading = angPos + inputString.substring(2, inputString.length()).toInt(); // get string after 'ah'
    ...
    {
      setAbsDisp = relativePositionVector + inputString.substring(2, inputString.length()).toInt(); // get string after 'rd'
    ...
    else if(inputString.substring(0,2) == "wp") // relative displacement, given in mm
    {
      int wpNum = (int) round( inputString.substring(2, inputString.length()).toInt() ); //  get string after 'wp'
...
//  posVec += relativePositionVector; // rectilinear translation assumption
//  Serial.print("r_{");
//  Serial.print(instance)
//  Serial.print("}^{(A)} = r_{");
//  Serial.print(prevInstance);
//  Serial.print("}^{(A)} + r_{");
//  Serial.print(instance);
//  Serial.print("}^{(A_{");
//  Serial.print(instance); 
...
//  Serial.print(prevInstance);
//  Serial.print("})} = ");
//  Serial.print(prevPosVec);
//  Serial.print(" mm + ");
//  Serial.print(relativePositionVector);
//  Serial.print(" mm = ");
//  Serial.print(posVec);
//  Serial.println(" mm");

  relativePositionVector 
            = (int) round( minLinearRes * ( pulses[0] + pulses[1] ) / 2.0 ); // r_{A_j/A_{j-1}}, rectilinear translation assumption
  Serial.print("r_{");
  Serial.print(instance);
  Serial.print("}^{(A_{");
  Serial.print(instance);
  ...
  Serial.print(" * ( pulses[0] + pulses[1] ) / 2.0 = "); 
  Serial.print(minLinearRes);
  Serial.print(" mm/pulse * ( ");
  Serial.print(pulses[0]);
  Serial.print(" pulses + ");
  Serial.print(pulses[1]);
  Serial.print(" pulses ) / 2.0 = ");
  Serial.print(relativePositionVector);
  Serial.println(" mm");
...
//  prevPosVec = posVec;
//  Serial.print("prevPosVec = posVec = ");
//  Serial.print(posVec);
//  Serial.println(" mm");
}
...
void readAngle()
{
  ...
  // compute this instant's theoretically instantaneous rate of ang pos change
  angDisp = (int) round( 360 * minLinearRes
                             * ( pulses[1] - pulses[0] ) 
                             / ( piApprox * botDiam ) ); // +CCW, d\theta, 2 * minLinearRes * ( pulses[1] - pulses[0] ) / botDiam; // theta_j^{(rel)} [rad]; before simplified: 2 * piApprox * minLinearRes * ( pulses[1] - pulses[0] ) / ( piApprox * botDiam )
  Serial.print("angDisp = 360 * minLinearRes * ( pulses[1] - pulses[0] ) / ( pi * system.botDiam )\n\t= 360 * ");
  Serial.print(minLinearRes);
  Serial.print(" mm/pulse * ( ");
  Serial.print(pulses[1]);
  Serial.print(" pulses - ");
  Serial.print(pulses[0]);
  Serial.print(" pulses ) / ( pi * ");
  Serial.print(botDiam);
  Serial.print(" mm ) = ");
  Serial.print(angDisp);
  Serial.println(" deg");

  Serial.print("angPos = ( angPos + angDisp ) % 360 = ( ");
  Serial.print(angPos);
  Serial.print(" deg + ");
  Serial.print(angDisp);
  Serial.print(" deg ) % 360"); // +CCW
  
  angPos = ( angPos + angDisp ) % 360; // [deg]
...
void readCoordinates()
{
  ...
  if( ( angPos >= 0 && angPos <= 90 ) || ( angPos >= 180 && angPos <= 270 ) ) // m >= 0
  {
    measDX = (int) round ( relativePositionVector * cos( angPos / degsPerRad ) );
    Serial.print("measDX = relativePositionVector * cos( angPos / degsPerRad ) = ");
    Serial.print(relativePositionVector);
    Serial.print(" * cos( ");
    Serial.print(angPos);
    Serial.print(" / ");
    ...
    measDY = (int) round ( relativePositionVector * sin( angPos / degsPerRad ) );
    Serial.print("measDY = relativePositionVector * sin( angPos / degsPerRad ) = ");
    Serial.print(relativePositionVector);
    Serial.print(" * sin( ");
    Serial.print(angPos);
    Serial.print(" / ");
  ...
  {
    measDX = (int) round ( -relativePositionVector * sin( angPos / degsPerRad ) );
    Serial.print("measDX = -relativePositionVector * sin( angPos / degsPerRad ) = -");
    Serial.print(relativePositionVector);
    Serial.print(" * sin( ");
    Serial.print(angPos);
    Serial.print(" / ");
    ...
    measDY = (int) round ( -relativePositionVector * cos( angPos / degsPerRad ) );
    Serial.print("measDY = -relativePositionVector * cos( angPos / degsPerRad ) = -");
    Serial.print(relativePositionVector);
    Serial.print(" * cos( ");
    Serial.print(angPos);
    Serial.print(" / ");
  ...
  measX += measDX; //(int) round ( measRelDisp * sin( -measRelHeading / degsPerRad ) ); // [mm]

  measY += measDY; //(int) round ( measRelDisp * cos( measRelHeading / degsPerRad ) ); // [mm]
...
{   
  ...
  else if(dx == 0)
  {
    if(dy > 0)
      ...
      setWyPtAbsHeading = 270; // or -90 deg
  }
  else if(dy == 0)
  {
    if(dx > 0)
  ...
  he1 = angPos - setWyPtAbsHeading; //( measAbsHeading - setWyPtAbsHeading ) % 360; // [deg]
  Serial.print("he1 = angPos - setWyPtAbsHeading = ");
  Serial.print(angPos);
  Serial.print(" deg - ");
  Serial.print(setWyPtAbsHeading);
  Serial.print(" deg = ");
...
//    }
    ...
    else // set displacement active
    {
      ...
      dispError = relativePositionVector - setAbsDisp; // [deg]
...
void rotateTask()
{
  ...
  { 
    ...
    else // set angle active
    {
      ...
      headingError = ( angPos - setAbsHeading ) % 360; // [deg]
